...
func (r *Report) latencies() []LatencyDistribution {
	percentiles := []int{10, 50, 75, 90, 99}
	data := make([]float64, len(percentiles))
	j := 0
	for i := 0; i < len(r.lats) && j < len(percentiles); i++ {
	...
	res := make([]LatencyDistribution, len(percentiles))
	for i := 0; i < len(percentiles); i++ {
		if data[i] > 0 {
			res[i] = LatencyDistribution{Percentage: percentiles[i], Latency: data[i]}
		}
	}
	return res
}
...
func histogram(buckets []Bucket) string {
	...
	for i := 0; i < len(buckets); i++ {
		...
		var barLen int
		if maxNum > 0 {
			barLen = (buckets[i].Count*40 + maxNum/2) / maxNum
		}
