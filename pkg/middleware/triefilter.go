...
var log = logger.WithField("service", "sensitive_filter")
...
func (st *Trie) FilterSpecialChar(text string) string {
...
// AddWord 添加敏感词
func (st *Trie) AddWord(sensitiveWord string) {
	// 添加前先过滤一遍
...
// AddWords 批量添加敏感词
func (st *Trie) AddWords(sensitiveWords []string) {
...
// Match 查找替换发现的敏感词
func (st *Trie) Match(text string) (sensitiveWords []string, replaceText string) {
	if st.root == nil {
		return nil, text
	}
...
// replaceRune 字符替换
func (st *Trie) replaceRune(chars []rune, begin int, end int) {
	for i := begin; i < end; i++ {
		chars[i] = st.replaceChar
	}
}

func Hans2Pinyin(contents []string) []string {
	pinyinContents := make([]string, 0)
	for _, content := range contents {
		hansReg := regexp.MustCompile("[\u4e00-\u9fa5]")
		if !hansReg.Match([]byte(content)) {
			continue
		}

		pin := pinyin.New(content)
		pinStr, err := pin.Convert()
		log.Debugln(content, "->", pinStr)
		if err == nil {
